...
export default {
  ...
  methods: {
    ...
    dataRefresh() {
      ...
      const ret = this.data_format(this.chartConfig, this.data, this.rcMeth);
      this.formatRet = ret || {};
      if (ret) {
        if (ret.cancel) {
          return;
    ...
    limitTextWidth(str, tw) {
      let s = '';
      for (let i = 0; i * tw < str.length; i += 1) {
    ...
    getXaxis(arr, pcnt, label, textw) {
      ...
      if (!label) {
        ...
        xa = arr.map((v, i, a) => (textw ? this.limitTextWidth(label(v, i, a), textw) : label(v, i, a)));
      }
      for (let i = arr.length; i < pcnt; i += 1) {
        xa.push('');
      }
      return xa;
    },
    mapAttribute(array, attribute, cb, origin = false) {
      ...
      // }
      const getValue = (v, i, a) => (cb ? cb(v[a], v, i, a) : v[a]);
      let rarr = [];
      if (array) {
        if (origin) {
          rarr = array.map((val, ind, arr) => {
            const ret = getValue(val, ind, attribute);
          ...
          rarr = array.map((val, ind, arr) => getValue(val, ind, attribute));
        }
      }
      return rarr;
    },
    ...
    setZoomTo(to, dindex = 0, zindex = 0) { // 注意在chartRefresh执行前数据可能和外部不一致
      const eo = this.getEOption();
      const dataLen = eo.series[dindex].data.length;
      const dz = eo.dataZoom[zindex] || eo.dataZoom;
      const s = dz.startValue; const e = dz.endValue;
      const pageCnt = e - s + 1;
      const [ns, ne] = this.zoomTo(dataLen, pageCnt, to);
    ...
    setLazyZoom(to, dindex = 0, zindex = 0) { // 注意在chartRefresh执行前数据可能和外部不一致
      ...
      const s = dz.startValue; const e = dz.endValue;
      const pageCnt = e - s + 1;
      const [ns, ne] = this.lazyZoom(dataLen, pageCnt, s, to);
      this.setZoom(ns, ne, zindex);
    },
  },
  watch: {
